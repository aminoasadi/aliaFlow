...
function MultiSelectDropdown({ field, label, options, value, onChange, error }: { field: string; label: string; options: Option[]; value: string[]; onChange: (value: string[]) => void; error?: string }) {
  const selected = options.filter((option) => value.includes(option.value));
  const summary = selected.length ? `${selected.length} selected` : "Select one or more";
...
export function BriefForm() {
  ...
  function validate(event: FormEvent<HTMLFormElement>) {
    ...
    if (!objective.length) nextErrors.hdf_prwzhh_ra_antkhab_kny = "Select at least one project objective.";
    if (!approach.length) nextErrors.rwykrd_rsydn_bh_hdf_ra_an = "Select at least one approach.";
    if (!touchpoint.length) nextErrors.nqth_tmas_kanal_ya_khrwjy = "Select at least one touchpoint, channel, or deliverable.";
  ...
  return <section id="brief-form" className="brief-form-section" aria-labelledby="brief-form-title">
    <div className="brief-form-intro"><p className="eyebrow">START A CONVERSATION</p><h2 id="brief-form-title">Brief Form</h2><p>Share your needs so we can shape the right conversation.</p></div>
    <div id="mauticform_wrapper_briefform" className="brief-form-wrapper mauticform_wrapper">
      <form autoComplete="off" role="form" method="post" action="http://crm.houseoftechnocrats.ir/form/submit?formId=18" id="mauticform_briefform" data-mautic-form="briefform" encType="multipart/form-data" onSubmit={validate}>
        ...
        <div className="mauticform-innerform"><div className="mauticform-page-wrapper mauticform-page-1" data-mautic-form-page="1">
          <label className="brief-field mauticform-row mauticform-email mauticform-required" htmlFor="mauticform_input_briefform_aymyl_khwd_ra_ward_knyd"><span className="brief-label mauticform-label">Email Address</span><input type="email" name="mauticform[aymyl_khwd_ra_ward_knyd]" id="mauticform_input_briefform_aymyl_khwd_ra_ward_knyd" className="mauticform-input" required /><span className="mauticform-errormsg" aria-live="polite" /></label>
          <label className="brief-field mauticform-row mauticform-text mauticform-required" htmlFor="mauticform_input_briefform_nam_w_nam_khanwadgy_khwd"><span className="brief-label mauticform-label">Full Name</span><input type="text" name="mauticform[nam_w_nam_khanwadgy_khwd]" id="mauticform_input_briefform_nam_w_nam_khanwadgy_khwd" className="mauticform-input" required /><span className="mauticform-errormsg" aria-live="polite" /></label>
          <MultiSelectDropdown field="hdf_prwzhh_ra_antkhab_kny" label="Why" options={objectives} value={objective} onChange={setObjective} error={errors.hdf_prwzhh_ra_antkhab_kny} />
          <MultiSelectDropdown field="rwykrd_rsydn_bh_hdf_ra_an" label="How" options={approaches} value={approach} onChange={setApproach} error={errors.rwykrd_rsydn_bh_hdf_ra_an} />
          <MultiSelectDropdown field="nqth_tmas_kanal_ya_khrwjy" label="What" options={touchpoints} value={touchpoint} onChange={setTouchpoint} error={errors.nqth_tmas_kanal_ya_khrwjy} />
          <div id="mauticform_briefform_submit" className="mauticform-row mauticform-button-wrapper"><button className="mauticform-button brief-submit" name="mauticform[submit]" value="1" id="mauticform_input_briefform_submit" type="submit">Submit Brief <span aria-hidden="true">↗</span></button></div>
        </div></div>
