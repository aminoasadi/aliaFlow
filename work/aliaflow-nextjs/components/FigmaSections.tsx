...
export function EventPromo({ title, image, imageAlt, kicker, body, ctaLabel, ctaHref, dark = false }: { title: string; image: string; imageAlt: string; kicker: string; body: string; ctaLabel: string; ctaHref: string; dark?: boolean }) {
  ...
  const displayImage = isFutureLeadershipJam ? "/assets/future-leadership-jam.svg" : isTechnocraticLeadershipJam ? "/assets/technocratic-leadership-jam.png" : image;
  const displayKicker = isFutureLeadershipJam || isTechnocraticLeadershipJam ? "Mon, Oct 13, 2025 – Fri, Oct 17, 2025" : kicker;
  const displayBody = (isFutureLeadershipJam || isTechnocraticLeadershipJam) && !body.trim()
    ? "Lorem ipsum dolor sit amet, consevbi hgseif adipiscing, sed do eismod hgseif adipiscing elit. Lorem ipsum dolor sit amet, consevbi hgseif adipiscing, sed do eismod sevbi hgseif adipiscing."
    : body;
  return <section className={`event-promo ${dark ? "event-promo-dark" : ""}${isFutureLeadershipJam ? " event-promo--future-leadership" : ""}${isTechnocraticLeadershipJam ? " event-promo--technocratic-leadership" : ""}`}><div className="event-copy"><h2>{title}</h2><p>{displayKicker}</p><p>{displayBody}</p><a className="event-cta" href={ctaHref}>{ctaLabel}</a></div><div className="event-image"><Image src={displayImage} alt={imageAlt} fill sizes="60vw" /></div></section>;
}
  ...
  return <>
    ...
    {statements.map((statement) => {
      ...
      return <Fragment key={statement.number}>
        ...
        <ThreeCards dark items={cards.map((card) => ({
          title: card.title,
          image: isBusinessGame
            ? "/assets/business-game-card.png"
            : isStrategicRoles
              ? "/assets/strategic-role-card.png"
              : isLeadershipModel
                ? "/assets/leadership-model-card.png"
                : card.image,
        }))} />
      </Fragment>;
    })}
  ...
  return <>
    ...
    {statements.map((statement) => {
      ...
      return <Fragment key={statement.number}>
        ...
        <ThreeCards items={cards.map((card) => ({
          title: card.title,
          image: isChangeSolving ? "/assets/change-solving-card.png" : isRiskSetting ? "/assets/risk-setting-card.png" : isPerformanceTesting ? "/assets/performance-testing-card.png" : card.image,
        }))} />
      </Fragment>;
    })}
...
export function WhyChooseUs({
  ...
  return <section className="why-us">
    ...
    <p>{eyebrow}</p>
    <h2><Lines text={displayHeading} /></h2>
    <div className="business-ring"><img src="/assets/why-thrivability-ring.svg" alt={ring_center_label.replace("\n", " ")} /></div>
    <div className="why-list">
      <article><img src="/assets/why-benefits.svg" alt="Different, Competitive, and Scalable benefits" /></article>
    </div>
  </section>;
}
...
export function PortfolioAndPeople({
  ...
  const toolkitImages = [
    "/assets/toolkit-1.png",
    "/assets/toolkit-2.png",
    "/assets/toolkit-3.png",
    "/assets/toolkit-4.png",
  ];
  ...
  return <>
    ...
    <div className="toolkits">
      {toolkits.map((toolkit, index) => <article key={toolkit.title}><Image src={toolkitImages[index] ?? toolkitImages[0]} alt={toolkit.title} fill sizes="(max-width: 780px) 100vw, 25vw" /></article>)}
    </div>
    </section>
  </>;
}
...
export function TestimonialsAndFooter({
  ...
  const partnerArtwork: Record<string, string> = {
    "Tehran University": "/assets/partner-tehran-university.png",
    "Raad Architect": "/assets/partner-raad-architect.png",
  };
  ...
  return <>
    <section className="partners">
      ...
      <div>{partners.map((partner, i) => {
        const artwork = partnerArtwork[partner.name];
        return <span className={artwork ? "partner-card--art" : undefined} key={`${partner.name}-${i}`}>{artwork ? <Image src={artwork} alt={partner.name} fill sizes="(max-width: 780px) 50vw, 12.5vw" /> : <><i />{partner.name}</>}</span>;
      })}</div>
